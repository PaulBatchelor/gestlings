...
function BlipSqueak.load_components(comp, top_path)
...
function BlipSqueak.components(o)
    local comp = {}
    o = o or {}
...
function BlipSqueak.speak(comp, phrase, pitchseq, temposeq)
    ...
    local words = {}
    local data = comp.data.blipsqueak
    ...
    for _, wrd in pairs(phrase) do
        table.insert(phrase_morphs, data.words[wrd])
    end

    SEQ = mcat(phrase_morphs)
    mseq = resolve(SEQ, data.vocab)
...
function BlipSqueak.load_data(comp, top_path)
    top_path = top_path or ""
    if comp.data == nil then comp.data = {} end

    assert(comp.data.blipsqueak == nil, "blipsqueak seems to be already loaded")
    assert(comp.asset ~= nil, "Could not find asset")

    comp.data.blipsqueak = {
        vocab = comp.asset:load(top_path .. "blipsqueak/morphemes.b64"),
        words = comp.asset:load(top_path .. "blipsqueak/words.b64")
    }
end
